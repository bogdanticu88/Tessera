...
2. `ClientOperations` directly, in-process, no HTTP: onboard, kill, restore, get, and the invariants that matter (idempotent kill, no resurrecting a killed client through onboard, get distinguishing an invalid client_ref from one that's just never been onboarded).
3. `LiveService_OnboardKillRestoreOverRealHttp`: builds `Tessera.Service` first (`dotnet build src/Tessera.Service`), then spawns it as a real subprocess on a random free port, waits for `/healthz`, and drives onboard, get, kill, and restore over actual HTTP with an actual signed JWT, plus the unauthenticated and not-found cases for get. This exercises the real ASP.NET pipeline, the auth filter, JSON binding, all of it, not just the logic underneath.
